...
def fetch_resources():
    ...
    logger.info(f"📥 Request received: type={resource_type}, lat={user_lat}, lon={user_lon}")

    # Fetch all resources of that type
    resources = get_resources(resource_type)
    logger.info(f"📦 Total resources with '{resource_type}': {len(resources)}")
    ...
    if user_lat is not None and user_lon is not None:
        ...
        for r in resources:
            try:
                ...
                logger.warning(f"❌ Error for {r.get('site_name', 'unknown')}: {e}")

        resources = sorted(filtered, key=lambda x: x["distance"])
        logger.info(f"✅ Found {len(resources)} nearby shelters:")
        for i, r in enumerate(resources[:5]):
            logger.info(f"{i+1}. {r['site_name']} - {r['distance']:.2f} miles away")
