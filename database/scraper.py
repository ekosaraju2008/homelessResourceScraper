...
def analyze_services(text_content, summary=None):
    ...
    try:
        ...
        - clothing (clothing closet, clothing donations, free clothes)
        ...
        service_keywords = {
            ...
            'clothing': ['clothing', 'clothes', 'clothing closet', 'clothing donations', 'free clothes', 'clothing bank']
        }
...
def get_shelter_urls(city_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # First check if there are no listings in this city
            content = soup.get_text().lower()
            if "no listing located within" in content or "no listings located within" in content:
                logger.info(f"No listings within city limits: {city_url}")
                return []
           
            shelter_sections = soup.select("div.layout_post_2.clearfix")
           
            for section in shelter_sections:
                # Get the text of the section to check location
                section_text = section.get_text().lower()
               
                # Skip if shelter is outside city limits (contains "miles outside" or similar phrases)
                if any(phrase in section_text for phrase in ["miles outside", "miles from", "miles away"]):
                    logger.info(f"Skipping shelter outside city limits: {section_text[:100]}...")
                    continue
                ...
                if shelter_link:
                    ...
                    # Add to shelter_urls if this is the first time we're seeing it
                    if shelter_url not in SEEN_SHELTER_URLS:
                        shelter_urls.append({
                            'directory_url': shelter_url,
                            'website': website_link
                        })
                        SEEN_SHELTER_URLS.add(shelter_url)
                        logger.info(f"Added new shelter URL: {shelter_url}")
                    else:
                        logger.info(f"Already processed shelter: {shelter_url}")
                   
        logger.info(f"Retrieved {len(shelter_urls)} new shelter URLs from {city_url}")
        return shelter_urls
...
def scrape_city_shelters(city, state):
    """Scrape shelter information for a specific city."""
    try:
        # Initialize set to track unique shelter names/addresses
        seen_shelters = set()
       
        # Format city URL for homelessshelterdirectory.org
        city_formatted = city.lower().replace(' ', '-')
        city_url = f"{BASE_URL}/city/{state.lower()}/{city_formatted}"
       
        # Get shelter URLs for this city
        shelter_urls = get_shelter_urls(city_url)
       
        # Process shelter listings
        shelters = []
        for shelter in shelter_urls:
            # Create a unique identifier using directory URL
            shelter_id = shelter['directory_url']
           
            # Skip if we've seen this shelter before
            if shelter_id in seen_shelters:
                logger.info(f"Skipping duplicate shelter at {shelter_id}")
                continue
               
            # Add to seen shelters
            seen_shelters.add(shelter_id)
           
            # Process shelter details
            shelter_info = scrape_service_data(shelter)
            if shelter_info:
                shelter_info['city'] = city
                shelter_info['state'] = state
...
def process_cities(batch_size=10):
    ...
    for city_url in city_links[:MAX_ITERATIONS]:
        try:
            ...
            shelter_urls = get_shelter_urls(city_url)
            if not shelter_urls:
                print(f"ℹ️ No valid shelters in {city}")
                continue
    ...
    if all_services:
        ...
        print(f"\n📊 Statistics:")
        print(f"   - Total unique shelters processed: {len(final_df)}")
